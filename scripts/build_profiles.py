...
def service_group(name: str, *, apple: bool = False, allow_reject: bool = False) -> str:
    ...
    """
    if apple:
        return select_group(name, "🚀 节点选择", "♻️ 自动选择", "🚀 手动切换", "DIRECT")
    members = ["♻️ 自动选择", "🚀 节点选择", "🚀 手动切换", "DIRECT"]
    if allow_reject:
        members.append("REJECT")
    return select_group(name, *members)
...
def render_groups(spec: ProfileSpec) -> list[str]:
    ...
    if tier == "mini":
        groups = [
            ...
            select_group("🐟 漏网之鱼", "♻️ 自动选择", "🚀 节点选择", "DIRECT", include_all=True),
        ]
    else:
        groups = [
            select_group("🚀 节点选择", "♻️ 自动选择", "🚀 手动切换", "DIRECT"),
            auto_group(),
            select_group("🚀 手动切换", include_all=True),
            service_group("📢 谷歌服务"),
        ...
        if tier == "full":
            groups.extend(
                [
                    select_group("📺 哔哩哔哩", "DIRECT", "♻️ 自动选择", "🚀 节点选择", "🚀 手动切换"),
                    service_group("💹 加密货币"),
                    service_group("🎮 游戏平台"),
                    service_group("💻 开发者服务"),
        ...
        groups.extend(
            [
                select_group("🎯 中国直连", "DIRECT", "♻️ 自动选择", "🚀 节点选择"),
                select_group("🐟 漏网之鱼", "♻️ 自动选择", "🚀 节点选择", "🚀 手动切换", "DIRECT"),
            ]
        )
    if spec.adblock:
