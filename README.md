...
面向后疫情互联网新时代的 Subconverter / Mihomo / Clash 现代分流规则。

疫情之后互联网格局剧变——AI 应用爆发、远程协作常态化、通讯工具多元化、
流媒体全球化、加密货币主流化。NEW-GEN Rules 为这些新时代服务提供精准分流，
而不是沿用十年前的过时规则。
...
欢迎提出任何问题、建议和使用反馈——规则只有在真实使用中才会越变越准。

- 某个服务分流不对（该直连的走了代理，或该代理的走了直连）；
- 某个新域名、新服务没有被覆盖；
- 想新增分组，或觉得某个分组该合并 / 拆分；
- 对规则维护、命名、结构有任何想法。

都可以提出来一起讨论，每一条真实反馈都会让这套规则更贴近实际使用。
